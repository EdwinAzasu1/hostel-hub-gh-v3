...
import { Building2, Plus, Edit2, Trash2, Eye, Clock, CheckCircle, XCircle, LogOut, MapPin } from 'lucide-react';
...
const OwnerDashboard = () => {
  ...
  const [hostels, setHostels] = useState<(Hostel & { status?: string })[]>([]);
  ...
  const fetchHostels = async (uid: string) => {
    try {
      ...
      const mapped = (hostelsData || []).map((hostel: any) => ({
        ...
        createdAt: new Date(hostel.created_at), updatedAt: new Date(hostel.updated_at), status: hostel.status,
      }));
      setHostels(mapped);
    } catch {
  ...
  const getStatusBadge = (status?: string) => {
    switch (status) {
      case 'pending':
        return <Badge variant="secondary" className="gap-1 text-xs"><Clock className="h-3 w-3" />Pending Review</Badge>;
      case 'approved':
        return <Badge className="gap-1 text-xs bg-success/15 text-success border border-success/30"><CheckCircle className="h-3 w-3" />Approved</Badge>;
      case 'rejected':
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent className="rounded-2xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will permanently delete this hostel and all associated room types.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
